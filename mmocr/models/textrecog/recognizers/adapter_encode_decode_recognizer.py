...
class AdapterEncodeDecodeRecognizer(BaseRecognizer):
    ...
    def __init__(
        ...
        init_cfg: InitConfigType = None,
        task_id=None,
    ) -> None:
        super().__init__(init_cfg=init_cfg, data_preprocessor=data_preprocessor)

        assert task_id is not None, "task_id should be specified"
    ...
    def extract_feat(self, inputs: torch.Tensor, task_id) -> torch.Tensor:
        """Directly extract features from the backbone."""
        if self.with_preprocessor:
            inputs = self.preprocessor(inputs)
        if self.with_backbone:
            inputs = self.backbone(inputs, task_id=task_id)
        return inputs

    def loss(
        self,
        inputs: torch.Tensor,
        data_samples: RecSampleList,
        task_id: int = None,
        **kwargs,
    ) -> Dict:
        ...
        feat = self.extract_feat(inputs, task_id=task_id)
        out_enc = None
    ...
    def predict(
        ...
        **kwargs,
        ...
        if self.with_encoder:
            out_enc = self.encoder(feat, data_samples)
        return self.decoder.predict(feat, out_enc, data_samples, task_id=task_id)
    ...
    def _forward(
        ...
        **kwargs,
        ...
        if self.with_encoder:
            out_enc = self.encoder(feat, data_samples)
        return self.decoder(feat, out_enc, data_samples, task_id)
